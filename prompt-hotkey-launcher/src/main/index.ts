...
app.on('ready', async () => {
  ...
  // Copy sample prompts if first run
...
app.on('window-all-closed', (e: Event) => {
  // Prevent app from quitting when all windows are closed (it's a menu bar app)
  e.preventDefault();
});
...
function setupIPC(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.INSERT_PROMPT, async (_, text: string) => {
    hidePopup();
    // Small delay to ensure popup is hidden and previous app is focused
